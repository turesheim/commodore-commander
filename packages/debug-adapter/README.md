...
The e2e suite launches the adapter over stdio, talks DAP as a debugger client,
and lets the adapter launch a real VICE process with the binary monitor. By
default it looks for the bundled macOS Apple Silicon VICE resources at
`packages/theia-extension/assets/vice/darwin-arm64/VICE.app/Contents/Resources`
and runs `x64sc`. Override those with `VICE_RESOURCES_PATH`,
`VICE_EXECUTABLE`, and optional `VICE_ARGS` when running against another VICE
build or CI installation. Failed runs write DAP transcripts and adapter output
under `test-results/vice-e2e/`.
