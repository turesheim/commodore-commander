...
export function createViceMonitorLabelCommands(
  ...
  const labels = (debugInfo?.labels ?? [])
    .map((label) => ({
      ...
      name: toViceMonitorLabelName(label.name)
    }))
    .filter((label): label is { address: number; name: string } =>
  ...
  const commands = [
    '; Generated by Commodore Commander from Kick Assembler debug info.'
  ];

  for (const label of labels) {
    const key = `${label.address}:${label.name.toLowerCase()}`;
    if (seen.has(key)) {
      continue;
    }
    seen.add(key);
...
function toViceMonitorLabelName(name: string): string | undefined {
...
function formatViceAddress(address: number): string {
  return (address & 0xffff).toString(16);
}
