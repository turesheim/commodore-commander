		...
		if (hoverRegion != null) {
			try {
				String hoveredText = textViewer.getDocument().get(hoverRegion.getOffset(), hoverRegion.getLength())
						.toUpperCase();
				IOMap.Entry entry = CommodoreCommanderPlugin.getPlugin().getIOMap().getEntriesMap().get(hoveredText);
				if (entry != null) {
					return getHTML(entry);
				}
				Mnemonics.Mnemonic mnemonic = CommodoreCommanderPlugin.getPlugin().getMnemonics().getMnemonicMap()
						.get(hoveredText);
				if (mnemonic != null) {
